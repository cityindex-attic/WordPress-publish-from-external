 ...
 * 
 * Adding a new content process involves a number of files all in their
 * respective folders. The css|js files would be optional. All files follow
 * strict filename formats.
 * 
 * The format of adding an action is:
 * - The namespace is in the format {sourceData}_to_{resultData} for example, 
 ...
 *   the format {$namespace} => "tab text"
 * - Add any js and css files to POSTEDITOR_DIR . /public_html/js|css in the
 *   format PosteditorModal_{$namespace}.js|css respectfully. Register these in
 ...
 * - The view file should be placed in POSTEDITOR_DIR . /public_html/ in the
 *   format PosteditorModal_{$namespace}.php
 *
 * @todo Create a parent class to handle the methods:
 * - get_page()
 * - get_styles()
 * - get_scripts()
 * @author daithi
...
class PosteditorModal {
	...
	public function __construct() {
		...
		$this->actions = array(
			...
			'upload_to_html' => 'Upload File (data > HTML)',
			'' => 'Extension Title'
		);
		$this->html = "";
	...
	private function get_tabs(){
		...
		foreach($this->actions as $action=>$tab){
			...
			$html .= "<li><a {$class} href=\"/labs-team/wp-admin/admin-ajax.php?action=get_modal_editor&posteditormodal_action={$action}&_wpnonce={$_REQUEST['_wpnonce']}&TB_iframe=true\">{$tab}</a></li>\n";
		}
